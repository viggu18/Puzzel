...
export default function Orders({route,navigation}) {
  ...
  const getCart = async () => {
    const docRef = db.collection("users/"+user.email+"/wishlist");
  ...
  return (
    ...
    <View style={{flex:1}}>
      ...
      <>
      <ViewCart navigation={navigation} data={Phones} cartValue={cartTotal} setCartValue={setCartTotal} seeOrders display/>
      <BottomBar title='Proceed' navigation={navigation} 
        CallingPage='cart' data={Phones} message={true} cartValue={cartTotal}/>
      </>
    </View>
    )
  }
